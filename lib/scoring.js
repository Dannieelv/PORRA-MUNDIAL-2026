...
// Deadline = medianoche del día ANTERIOR al partido (~24-48h antes del partido)
...
export function getCountdown(match) {
  ...
  const veryUrgent = diff < 7200000;  // < 2h
  let text;
  if (days > 0)       text = `${days}d ${hours}h`;
  else if (hours > 0) text = `${hours}h ${mins}m`;
  else                text = `${mins}m`;
...
export function isMatchLocked(match, locked) {
  ...
  return new Date() >= getDeadline(match);
}
