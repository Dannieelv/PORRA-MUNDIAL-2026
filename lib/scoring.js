...
// Deadline para apostas de eliminatorias = 2h antes del partido
export function getKnockoutDeadline(match) {
  if (match.datetime) {
    return new Date(new Date(match.datetime).getTime() - 2 * 3600 * 1000);
  }
  // Fallback: 2h antes de medianoche del día del partido
  return new Date(match.date + 'T22:00:00Z');
}

export function isKnockoutLocked(match) {
  return new Date() >= getKnockoutDeadline(match);
}

export function getKnockoutCountdown(match) {
  const deadline = getKnockoutDeadline(match);
  const diff = deadline - new Date();
  if (diff <= 0) return { expired: true, text: 'Cerrado', urgent: false, veryUrgent: false };
  const days  = Math.floor(diff / 86400000);
  const hours = Math.floor((diff % 86400000) / 3600000);
  const mins  = Math.floor((diff % 3600000) / 60000);
  const urgent     = diff < 86400000;
  const veryUrgent = diff < 7200000;
  let text;
  if (days > 0)       text = `${days}d ${hours}h`;
  else if (hours > 0) text = `${hours}h ${mins}m`;
  else                text = `${mins}m`;
  return { expired: false, text, urgent, veryUrgent };
}

export function knockoutMatchPoints(pred, result, mult, points = DEFAULT_POINTS) {
  ...
  // Quién clasificó realmente (winner tiene prioridad para cubrir penaltis)
  let realW;
  if      (result.winner === 'HOME_TEAM') realW = 'h';
  else if (result.winner === 'AWAY_TEAM') realW = 'a';
  else realW = rh > ra ? 'h' : ra > rh ? 'a' : 'd';

  const predW = ph > pa ? 'h' : pa > ph ? 'a' : 'd';

  // Sign: acertó quién clasifica (pens incluidos)
  if (predW === realW) pts += P.knockoutSign ?? 3;

  // Diff/Exact: aplican si acertó el ganador, O si el partido acabó en empate a 90min
  // y el usuario también predijo empate (predijo el marcador de 90min correctamente)
  const isDraw90 = rh === ra;
  if (predW === realW || (isDraw90 && predW === 'd')) {
    if ((ph - pa) === (rh - ra)) pts += P.knockoutDiff  ?? 1;
    if (ph === rh && pa === ra)  pts += P.knockoutExact ?? 5;
  }

  // Total: mismos goles totales, siempre
  if ((ph + pa) === (rh + ra)) pts += P.knockoutTotal ?? 1;
  return pts * (mult || 1);
}
...
export function playerScore(player, config) {
  ...
  (config?.knockoutMatches || []).forEach(m => {
    ...
    const result = (config.knockoutResults || {})[m.id];
    kp += knockoutMatchPoints(pred, result, m.mult, points);
  });

  return { match: mp, clasif: cp, tournament: tp, knockout: kp, total: mp + cp + tp + kp };
}
...
// Deadline = medianoche de inicio del día del partido (00:00 del día del partido)
// La apuesta cierra cuando queden exactamente 24h para ese momento
// = medianoche del día anterior al partido
export function getDeadline(match) {
  return new Date(match.date + 'T00:00:00');
}

export function getCountdown(match) {
  const deadline = getDeadline(match);
  const diff = deadline - new Date();
  // La apuesta cierra cuando queden 24h para la medianoche del partido
  const lockDiff = diff - 24 * 3600000;
  if (lockDiff <= 0) return { expired: true, text: 'Cerrado', urgent: false, veryUrgent: false };
  const days  = Math.floor(lockDiff / 86400000);
  const hours = Math.floor((lockDiff % 86400000) / 3600000);
  const mins  = Math.floor((lockDiff % 3600000) / 60000);
  const urgent     = lockDiff < 7200000;  // aviso solo últimas 2h
  const veryUrgent = lockDiff < 3600000;  // rojo último hora
  let text;
  if (days > 0)       text = `${days}d ${hours}h`;
  else if (hours > 0) text = `${hours}h ${mins}m`;
  else                text = `${mins}m`;
  return { expired: false, text, urgent, veryUrgent, days, hours, mins };
}

export function isMatchLocked(match, locked) {
  if (locked) return true;
  // Cierra cuando queden ≤ 24h para la medianoche del día del partido
  return (getDeadline(match) - new Date()) <= 24 * 3600000;
}
